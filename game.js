const VERSION = '1.00';
const STORAGE_KEY = 'mini_spire_save_v6';
...
function applyDrain(amount, unit) { return unit.drain > 0 ? Math.floor(amount * 0.75) : amount; }
function calcPlayerDamage(base) { return Math.max(0, applyDrain(base + playerStrength() + relicDamageBonus(), state.player)); }
function calcEnemyAttack(enemy, base) { return Math.max(0, applyDrain(base + (enemy.strength || 0), enemy)); }
function getCardRarityClass(id) { return `rarity-${cardDefs[id].rarity || 'common'}`; }
function getRelicRarityClass(r) { return `rarity-${r.rarity || 'common'}`; }
function cardLabel(id) { const c = cardDefs[id]; return `<span class="${c.upgraded ? 'upgraded' : ''}">${c.name}【${c.cost}】${c.type}</span>`; }
...
function gainBlock(unit, value) { const actual = unit.vuln > 0 ? Math.max(0, Math.floor(value * 0.75)) : value; unit.block += actual; }
...
function spawnNormalGroup() {
  ...
  addLog(`この戦闘の危険度目標: ${targetDanger}`);
  return group;
}
...
function startCombat(encounter = null) {
  ...
  state.mode = 'combat';
  state.turn = 1;
  state.player.block = 0; state.player.energy = 3; state.player.strengthCombat = 0; state.player.weak = 0; state.player.vuln = 0; state.player.drain = 0;
...
function playCard(index) {
  ...
  if (c.dmg && target) {
    ...
    for (let i = 0; i < hitCount; i += 1) { if (target.hp <= 0) break; damage(target, calcPlayerDamage(c.dmg)); }
  }
...
function pickRewardChoices(elite = false) {
  ...
  const pool = elite ? [...shuffle(high).slice(0, 9), ...shuffle(common).slice(0, 1)] : rewardCards;
...
function victory() {
  ...
  state.mode = 'reward';
  state.rewardTaken = false; state.relicTaken = !elite;
...
function useItem(type) {
  if (state.mode !== 'combat' || state.items[type] <= 0) return;
  if (type === 'healPotion') state.player.hp = Math.min(state.player.maxHp, state.player.hp + 18);
  if (type === 'bombPotion') aliveEnemies().forEach((e) => damage(e, 15));
  state.items[type] -= 1;
  if (aliveEnemies().length === 0) { victory(); return; }
  render();
}
...
function runEvent() {
  const r = Math.random();
  if (r < 0.34) {
    const commonRelic = availableRelics().find((x) => x.rarity === 'common');
    if (commonRelic) { state.player.relics.push(commonRelic); if (commonRelic.onGain) commonRelic.onGain(); addLog(`無色の宝箱: ${commonRelic.name}を獲得。`); }
    else addLog('無色の宝箱は空だった。');
  } else if (r < 0.67) {
    if (Math.random() < 0.5) { const heal = Math.ceil(state.player.maxHp * 0.2); state.player.hp = Math.min(state.player.maxHp, state.player.hp + heal); addLog(`回復の泉: HPを${heal}回復。`); }
    else { const inc = Math.ceil(state.player.maxHp * 0.1); state.player.maxHp += inc; state.player.hp += inc; addLog(`回復の泉: 最大HPが${inc}上昇。`); }
  } else {
    const lose = Math.ceil(state.player.maxHp * 0.12); const gain = 45;
    state.player.hp = Math.max(1, state.player.hp - lose); state.gold += gain;
    addLog(`血の祭壇: HP${lose}を失い、${gain}G獲得。`);
  }
  state.mode = 'map';
  prepareMap();
  render();
...
function statusLine(s, w, v, d) { const bits = []; if (s > 0) bits.push(`筋力:${s}`); if (w > 0) bits.push(`弱体:${w}`); if (v > 0) bits.push(`脆弱:${v}`); if (d > 0) bits.push(`脱力:${d}`); return bits.length ? `<p>${bits.join(' / ')}</p>` : ''; }
...
function renderHUD() { el.hud.innerHTML = `<div class="grid hud-grid-5"><div class="stat">HP: ${state.player.hp} / ${state.player.maxHp}</div><div class="stat">階層: ${state.floor} / ${state.maxFloor}</div><div class="stat">所持金: ${state.gold}G</div><button class="stat deck-btn" id="deckBtn">デッキ: ${state.player.deck.length}枚</button><button class="stat deck-btn" id="relicBtn">所持レリック一覧</button></div>${renderDeckList()}${renderRelicList()}`; document.getElementById('deckBtn').addEventListener('click', toggleDeck); document.getElementById('relicBtn').addEventListener('click', toggleRelics); }
...
function renderState() {
  ...
  if (state.mode === 'map') {
    ...
    el.state.innerHTML = `<h2>次の階層へ進む</h2><p>${state.nextNode === 'event' ? '奇妙な気配を感じる。' : (state.eliteOffer ? '強い気配を感じる。' : '次の敵を探して進む。')}</p>`;
    return;
  }
...
function renderActions() {
  if (state.mode === 'combat') {
    el.actions.innerHTML = `<div class="row">${state.player.hand.map((id, i) => `<button class="card-button ${getCardRarityClass(id)}${cardDefs[id].cost > state.player.energy ? ' is-disabled' : ''}" ${cardDefs[id].cost > state.player.energy ? 'disabled' : ''} data-play="${i}">${cardLabel(id)}<br><span class="small">${cardEffectText(id)}</span></button>`).join('')}</div><div class="row" style="margin-top:0.6rem;"><button id="endTurn">ターン終了</button><button id="useHeal" ${state.items.healPotion <= 0 ? 'disabled class="is-disabled"' : ''}>回復ポーション(${state.items.healPotion})</button><button id="useBomb" ${state.items.bombPotion <= 0 ? 'disabled class="is-disabled"' : ''}>爆発ポーション(${state.items.bombPotion})</button></div>`;
    el.actions.querySelectorAll('[data-play]').forEach((b) => b.addEventListener('click', () => playCard(Number(b.dataset.play))));
    document.getElementById('endTurn').addEventListener('click', endTurn);
    document.getElementById('useHeal').addEventListener('click', () => useItem('healPotion'));
    document.getElementById('useBomb').addEventListener('click', () => useItem('bombPotion'));
    return;
  }
  if (state.mode === 'map') {
    ...
    if (state.nextNode === 'event') { el.actions.innerHTML = '<button id="goEvent">イベントへ進む</button>'; document.getElementById('goEvent').addEventListener('click', runEvent); return; }
  ...
  if (state.mode === 'reward') {
    const relic = state.relicChoices[0]; const canProceed = state.rewardTaken && (state.relicTaken || !relic);
  ...
  if (state.mode === 'shop') { el.actions.innerHTML = `<div class="row">${state.shopChoices.map((c) => `<button data-buy="${c.id}" data-price="${c.price}" class="${getCardRarityClass(c.id)}">${cardLabel(c.id)}<br><span class="small">${cardEffectText(c.id)} / ${c.price}G</span></button>`).join('')}</div><div class="row" style="margin-top:0.6rem;"><button id="leaveShop">ショップを出る</button></div>`; el.actions.querySelectorAll('[data-buy]').forEach((b) => b.addEventListener('click', () => buyCard(b.dataset.buy, Number(b.dataset.price)))); document.getElementById('leaveShop').addEventListener('click', leaveShop); return; }
  if (state.mode === 'camp') { el.actions.innerHTML = `<div class="row"><button id="campHeal">休憩（14回復）</button></div><p class="small">カード削除（1枚）</p><div class="row">${state.campChoices.map((id) => `<button data-remove="${id}" class="${getCardRarityClass(id)}">削除: ${cardLabel(id)}</button>`).join('')}</div><p class="small">カード強化（1枚）</p><div class="row">${state.campChoices.filter((id) => cardDefs[id].upgradeTo).map((id) => `<button data-up="${id}" class="${getCardRarityClass(id)}">強化: ${cardLabel(id)}<br><span class="small">前: ${cardEffectText(id)}<br>後: ${cardEffectText(cardDefs[id].upgradeTo)}</span></button>`).join('')}</div>`; document.getElementById('campHeal').addEventListener('click', campHeal); el.actions.querySelectorAll('[data-remove]').forEach((b) => b.addEventListener('click', () => campRemove(b.dataset.remove))); el.actions.querySelectorAll('[data-up]').forEach((b) => b.addEventListener('click', () => campUpgrade(b.dataset.up))); return; }
